...
void query(const char* filename) {
   ...
   // ofstream myfile;
    ...
    vector<tuple> result1 = vector<tuple>();
    //conver results to vector 
    for (set<tuple>::iterator i = result.begin(); i != result.end(); i++) {
        result1.push_back(*i);
    }
    ...
    cout << "found " << result1.size() << " distinct two-paths.\n";
