...
class DatalogTest(unittest.TestCase):
  def test_join(self):
    ...
    desiredresult = """[('A', Project($0,$3)[Join($1 = $0)[Scan(public:adhoc:R), Scan(public:adhoc:S)]])]"""
    testresult = RATest(join)
    self.assertEqual(testresult, desiredresult)

  def test_selfjoin(self):
    join = """A(x,z) :- R(x,y), R(y,z)"""
    desiredresult = """[('A', Project($0,$3)[Join($1 = $0)[Scan(public:adhoc:R), Scan(public:adhoc:R)]])]"""
    testresult = RATest(join)
    self.assertEqual(testresult, desiredresult)

  #def test_triangle(self):
  #  join = """A(x,y,z) :- R(x,y), S(y,z), T(z,x)"""
  #  desiredresult = """[('A', Project($0,$1,$3)[Select($0 = $5)[Join($3 = $0)[Join($1 = $0)[Scan(public:adhoc:R), Scan(public:adhoc:S)], Scan(public:adhoc:T)]]])]"""
  #  testresult = RATest(join)
  #  self.assertEqual(testresult, desiredresult)

  def test_explicit_conditions(self):
    join = """A(x,y,z) :- R(x,y), S(w,z), x<y,y<z,y=w"""
    desiredresult = """[('A', Project($0,$1,$3)[Join($1 < $1 and $1 = $0)[Select($0 < $1)[Scan(public:adhoc:R)], Scan(public:adhoc:S)]])]"""
    testresult = RATest(join)
    self.assertEqual(testresult, desiredresult)
...
"""
    desiredresult = """[('JustXBillSquared', Project($0)[Join($0 = $0)[Apply(x=$0)[Project($0)[Scan(public:adhoc:TwitterK)]], Apply(x=$0)[Project($0)[Apply(x=$0)[Project($0)[Scan(public:adhoc:TwitterK)]]]]]])]"""
    testresult = RATest(query)
    self.assertEqual(testresult, desiredresult)
