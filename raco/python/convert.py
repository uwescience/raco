...
def convert(source_or_ast_or_callable, schema):
    """
    ...
    if isinstance(source_or_ast_or_callable, basestring):
        try:
            return convert(ast.parse(source_or_ast_or_callable), schema)
        except SyntaxError as e:
            raise PythonSyntaxException(e.msg, e.lineno, e.offset)
    elif callable(source_or_ast_or_callable):
        return convert(get_source(source_or_ast_or_callable),
                       schema)
    elif isinstance(source_or_ast_or_callable, ast.AST):
        return visitor.ExpressionVisitor(schema or []).visit(
